...
fwrite(Resultsd, "CI_behavioral.csv")
...
for (i in First:Last) {
  print(i)
  LMEOut <- withWarnings( a <- lmer(prs_baseline[,i] ~ non_cog_score + cog_score + C1 + C2 + C3 + C4 +
                                      C5 + C6 + C7 + C8 + C9 + C10 + sex + interview_age + (1|site_id) + (1|rel_family_id), 
...
#Store results in a dataframe
Results9_CI <- cbind.data.frame(Area[First:length(Area)], Beta[First:length(Beta)], STE[First:length(STE)], Pval[First:length(Pval)],  Beta2[First:length(Beta2)], STE2[First:length(STE2)], Pval2[First:length(Pval2)], CIl[First:length(CIl)], CIu[First:length(CIu)], CIl_2[First:length(CIl_2)], CIu_2[First:length(CIu_2)])
#Relabel Results
colnames(Results9_CI) <- c("Variable", "Beta_non_cog", "STE_non_cog", "Pval_non_cog", "Beta_cog", "STE_cog", "Pval_cog", "lower_non_cog", "upper_non_cog", "lower_cog", "upper_cog")
fwrite(Results9_CI, "CI_wealth.csv")
